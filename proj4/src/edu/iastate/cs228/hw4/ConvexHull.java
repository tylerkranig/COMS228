...
public abstract class ConvexHull 
{
	...
	public ConvexHull(Point[] pts) throws IllegalArgumentException 
	{
		...
		int i=0;
		for(Point n:pts){points[i]=n;i++;}
		//find lowest point
		lowestPoint=points[0];
		for(Point n:points){
			if(n.getY()<lowestPoint.getY()||(n.getY()==lowestPoint.getY()&&n.getX()<lowestPoint.getX())){lowestPoint=n;}
		}
		//initialize quicksorter
		quicksorter=new QuickSortPoints(points);
		//call removeDuplicates()
		removeDuplicates();
		//Set lowestPoint to pointsNoDuplicate[0]. 
		i=0;
		for(Point n:pointsNoDuplicate){
			if(n.equals(lowestPoint)){
				swap(0,i);
				break;
			}
		}
	...
	public ConvexHull(String inputFileName) throws FileNotFoundException, InputMismatchException
	{
		...
		ArrayBasedStack<Point> temp=new ArrayBasedStack<Point>();//used for adding to an array later, this way we dont have to calculate size ahead of time
		while(in.hasNext()){
			try{
				int x=in.nextInt();
				int y=in.nextInt();
			...
			catch(NoSuchElementException e){//catches an odd amount of numbers in the file
	...
	public String stats()
	{
		String out=String.format("%-20s%-10d", algorithm,points.length);
		out+=this.time;
		return out;
	}
	...
	public String toString()
	{
		String out="";
		int i=1;
		for(Point n:this.hullVertices){
			out+=String.format("%s   ", n.toString());
			if(i==5){
				out+="\n";
				i=0;
			}
	...
	{
		if(hullVertices==null||hullVertices[0]==null)
			throw (new IllegalStateException());
		File out=new File("hull.txt");
		PrintWriter outer=new PrintWriter(out);
		for(Point n:hullVertices)
			outer.println(n.toString());
		outer.close();
	}
	...
	{		
		...
		for(i=0;i<hullVertices.length-1;i++){
			Segment seg=new Segment(hullVertices[i],hullVertices[i+1]);
			segments[i]=seg;
		}
		Segment seg=new Segment(hullVertices[i],hullVertices[0]);
		segments[i]=seg;
